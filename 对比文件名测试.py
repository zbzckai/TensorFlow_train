...
model_dir = './/1000_480_40000'
model_name = 'intermediate_20000.pb'
image_dir = r'./testimages'
label_dir = './'
...
def test_function(model_name):
    ...
    with tf.Session() as sess:
        ...
        for file in os.listdir(image_dir):
            ...
            pacture_imfor.append(re.sub("[A-Za-z0-9\!\%\[\]\,\。\(\)\（\）\_\.\ ]", "", file))
            # 载入图片
            image_data = tf.gfile.FastGFile(os.path.join(image_dir, file), 'rb').read()
        ...
        joblib.dump(solution, './/1000_480_40000/solution_{}.m'.format(model_name.split('.')[0]))

for model_name in [x for x in os.listdir(model_dir)[28::] if 'pb' in x]:
    #model_name = 'intermediate_59000.pb'
    print('===================================================================================================')
    print(model_name)
